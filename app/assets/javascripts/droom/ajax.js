  ...
  jQuery(function ($) {
    ...
    return (Remote = (function () {
      ...
      Remote.prototype.pend = function (event, xhr, settings) {
        ...
        return (ref =
          typeof (base = this._options).on_request === "function"
            ? base.on_request(xhr, settings)
            : void 0) != null
          ? ref
          : true;
      };
      ...
      Remote.prototype.fail = function (event, xhr, status) {
        ...
        if (xhr.status !== 409 && xhr.status !== 401) {
          ...
          let message =
            xhr.getResponseHeader("X-Flash-Message") ||
      ...
      Remote.prototype.receive = function (event, data, status, xhr) {
        // Get the explicit headers (no fallbacks here yet)
        const flashMessage = xhr.getResponseHeader("X-Flash-Message");
        const flashType = xhr.getResponseHeader("X-Flash-Type") || "notice";

        console.log("AJAX Response:", flashMessage, flashType);
        ...
        // ONLY notify if the server sent a message, OR if it's a write method (using fallback)
        if (flashMessage) {
          this.notify(flashMessage, flashType);
        } else if (writeMethods.includes(rawMethod)) {
          this.notify("Operation completed successfully.", "notice");
        }
